...
/** Durchsuchbare Seiten. Neue Seite? Hier einen Eintrag ergänzen. */
const SEARCHABLE_PAGES = [
  {
    title: 'Unsere Arbeiten (Portfolio)',
    path: '/portfolio',
    keywords: ['portfolio', 'arbeiten', 'projekte', 'works'],
  },
  {
    title: 'Wer sind wir (Info)',
    path: '/about',
    keywords: ['info', 'about', 'wer sind wir', 'über uns', 'agentur'],
  },
  {
    title: 'Kundenportal',
    path: '/login',
    keywords: ['kunden', 'portal', 'login', 'dashboard'],
  },
];
...
export default function SearchModal({ isOpen, onClose }: Props) {
  ...
  const handleSubmit = (e: FormEvent) => {
    e.preventDefault();
    if (results.length > 0) openPage(results[0].path);
  };
        ...
        >
          ...
          >
            <form
              ...
              className="flex items-center bg-[#EBEBEB] border-2 border-black rounded-full px-6 py-4 shadow-[0_8px_30px_rgb(0,0,0,0.12)]"
            >
            ...
            {query && results.length > 0 && (
              <div className="mt-4 bg-[#EBEBEB] border-2 border-black rounded-3xl p-6 shadow-lg">
                <h3 className="text-sm font-bold text-black/50 uppercase tracking-wider mb-4">
                  Ergebnisse
                </h3>
                <div className="space-y-2">
                  {results.map((page) => (
                    <button
                      ...
                      className="w-full flex items-center justify-between p-4 hover:bg-black/5 rounded-xl transition-colors text-left border border-transparent hover:border-black/10"
                    >
